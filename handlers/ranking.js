    ...
    client.on("messageCreate", async (message) => {
     try{
        ...
        function databasing(rankuser) {
            ...
            client.points.ensure(rankuser ? `${message.guild.id}-${rankuser.id}` : `${message.author.id}`, {
            ...
            client.points.set(rankuser ? `${message.guild.id}-${rankuser.id}` : `${message.author.id}`, rankuser ? rankuser.tag : message.author.tag, `usertag`); //set the usertag with EVERY message, if he has nitro his tag might change ;)
        ...
        function addrandomall() {
            ...
            for (let i = 0; i < allmembers.length; i++) {
                ...
                Giving_Ranking_Points(`${message.author.id}`, maxnum);
            }
            const embed = new Discord.MessageEmbed()
            .setColor(embedcolor)
